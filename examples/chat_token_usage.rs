...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let request = ChatRequest::builder(MODEL)
        .messages(vec![Message::user("In one sentence, why is the sky blue?")])
        .build();
    ...
    while let Some(chunk) = stream.next().await {
        ...
        if chunk.done {
            println!("\n\n--- usage ---");
            println!("prompt tokens:   {}", fmt_count(chunk.prompt_eval_count));
            ...
            println!("prompt eval:  {}", fmt_duration(chunk.prompt_eval_duration));
            println!("response gen: {}", fmt_duration(chunk.eval_duration));

            if let (Some(eval_count), Some(eval_duration)) = (chunk.eval_count, chunk.eval_duration)
                && eval_duration > 0
            {
                let tokens_per_sec = eval_count as f64 / (eval_duration as f64 / 1_000_000_000.0);
